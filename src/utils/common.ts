...
export const separateCustomerServerWise = (customerRecords: Customer[]) => {
  ...
  customerRecords.forEach(
    (c: Customer) =>
      (maxServerNumber =
        c.server! > maxServerNumber ? c.server! : maxServerNumber)
  );
...
export const generateNormalDistribution = (numOfCustomers: number) => {
  ...
  const meanService =
    data.shaped.reduce((acc, curr) => acc + curr.service, 0) /
    data.shaped.length;
  ...
  const meanInter =
    data.shaped.reduce((acc, curr) => acc + curr.interArrival, 0) /
    data.shaped.length;
  ...
  for (let i = 0; i < numOfCustomers; i++) {
    normalDistributionService.push(
      generateNormalDistributionNumber(meanService, stdDevService)
    );
    normalDistributionInter.push(
      generateNormalDistributionNumber(meanInter, stdDevInter)
    );
  }
...
export const getExpectedFrequenciesAndSummation = (
  observedFreqSummation: number,
...
export const getExpectedFrequencies = (
  data: number[],
  numIntervals: number,
  mean?: number
) => {
  ...
  for (let i = 0; i < numIntervals; i++) {
    ...
    const expectedFreq =
      data.filter((time) => time >= lowerBound && time < upperBound).length *
      numIntervals;
...
export const chiSquare = (
  ...
  for (let i = 0; i < expectedFrequencies.length; i++) {
    chiSquare +=
      Math.pow(Math.abs(observedFrequencies[i] - expectedFrequencies[i]), 2) /
...
export function poissonInterArrivals(lambda: number) {
  ...
  while (MaxCumulativeProbability > cumulativeProbability) {
    if (cumulativeProbability < MaxCumulativeProbability)
      interArrivals.push(cumulativeProbability);
...
export const getInterArrivalsFromRange = (
  commulativeProbabilities: number[]
) => {
  // console.log('====================================');
   ...
   * ]
   */
  const ranges = commulativeProbabilities
    .map((num, i, arr) => [num, arr[i + 1]])
    .filter((range) => range[1]); // create ranges
